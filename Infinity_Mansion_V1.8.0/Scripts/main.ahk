...
readme.Add("Text", "x30 y20 w740 h40", "Infinity Castle Macro V1.8.0")
...
readme.Add("Text", "x50 y510 w700 h20", "• Disable Auto-Open Upgrade UI: On")
...
main.Add("Text", "x10 y250 w200 h20 cFFFFFF", "Made By Thuy | V1.8.0")
...
Checkbox2 := main.Add("Checkbox", "x10 y60 w190 h20 cFFFFFF", "Unit Slot 2")
Dropdown2 := main.Add("DropDownList", "x225 y60 w50 h20 Background333333 cFFFFFF r6", ["1", "2", "3", "4", "5", "6"])

Checkbox3 := main.Add("Checkbox", "x10 y90 w190 h20 cFFFFFF", "Unit Slot 3")
Dropdown3 := main.Add("DropDownList", "x225 y90 w50 h20 Background333333 cFFFFFF r6", ["1", "2", "3", "4", "5", "6"])

Checkbox4 := main.Add("Checkbox", "x10 y120 w190 h20 cFFFFFF", "Unit Slot 4")
Dropdown4 := main.Add("DropDownList", "x225 y120 w50 h20 Background333333 cFFFFFF r6", ["1", "2", "3", "4", "5", "6"])

Checkbox5 := main.Add("Checkbox", "x10 y150 w190 h20 cFFFFFF", "Unit Slot 5")
Dropdown5 := main.Add("DropDownList", "x225 y150 w50 h20 Background333333 cFFFFFF r6", ["1", "2", "3", "4", "5", "6"])

Checkbox6 := main.Add("Checkbox", "x10 y180 w190 h20 cFFFFFF Checked", "Unit Slot 6 (Use A Hill Unit)")
Dropdown6 := main.Add("DropDownList", "x225 y180 w50 h20 Background333333 cFFFFFF r6", ["1", "2", "3", "4", "5", "6"])

; Add controls to the options GUI
Checkbox7 := options.Add("Checkbox", "x10 y30 w150 h20 cFFFFFF", "Do Hardmode")
Checkbox8 := options.Add("Checkbox", "x10 y60 w200 h20 cFFFFFF", 'Click "Next Room" or "Retry"')

Editbox1 := options.Add("Edit", "x10 y120 w20 h20 Background333333 cFFFFFF", "Amount Of Times To Upgrade Hill/Hybrid")
...
if !FileExist("..\Settings\settings.ini") {
    ...
    IniWrite("1", "..\Settings\settings.ini", "Options", "Amount_Of_Times_To_Upgrade_Hill/Hybrid")
}
...
OpenOptions() {
    ...
    ; Add controls on top of the background
    Checkbox7 := options.Add("Checkbox", "x10 y10 w150 h20 cFFFFFF", "Do Hardmode")
    Checkbox7.Value := IniRead("..\Settings\settings.ini", "Options", "Hardmode")

    Checkbox8 := options.Add("Checkbox", "x10 y30 w200 h20 cFFFFFF", 'Click "Next Room" or "Retry"')
    Checkbox8.Value := IniRead("..\Settings\settings.ini", "Options", "Next_Room/Retry")

    ; Add the Text control before the Edit control
    options.Add("Text", "x65 y50 w200 h40 cFFFFFF Background1E1E1E", "<-- Amount Of Times To Upgrade Hill/Hybrid Per Cycle")

    ; Add an Edit control with a smaller font
    Editbox1 := options.Add("Edit", "x10 y50 w50 h20 Background333333 cFFFFFF", "1")
    Editbox1.Value := IniRead("..\Settings\settings.ini", "Options", "Amount_Of_Times_To_Upgrade_Hill/Hybrid")
    Editbox1.SetFont("s8 norm cFFFFFF", "Segoe UI")

    ; Validate input to only allow numbers >= 1
    Editbox1.OnEvent("Change", (*) => (
        ValidateEditbox1()
    ))
...
; Function to validate the Edit control
ValidateEditbox1() {
    global Editbox1

    ; Get the current value and convert it to a number
    Value := Number(Editbox1.Value)

    ; Check if the value is a number and >= 1
    if !(Value is number) || Value < 1 {
        ; If invalid, reset to the last valid value (default is "1")
        Editbox1.Value := 1
    } else {
        ; If valid, allow the value to be set
        Editbox1.Value := Value
    }
...
Unit_GUI_Save() 
{
    ...
    IniWrite(Editbox1.Value, "..\Settings\settings.ini", "Options", "Amount_Of_Times_To_Upgrade_Hill/Hybrid")
...
Upgrade_Unit_Slot_1(x1, y1, x2, y2, x3, y3) 
{
    ...
    Loop {
        if ceo_maxed_1 != 0 && ceo_maxed_2 != 0 && ceo_maxed_3 != 0{
            break
        }
...
Upgrade_Unit_Slot_2(x1, y1, x2, y2, x3, y3, x4, y4, x5, y5, x6, y6) 
{
    ...
    SendClick(x1, y1)
    Sleep(100)
    if ImageFound_hill_unit() || ImageFound_hybrid_unit() {
        SendClick(552, 354)
        Sleep(100)
        Loop Editbox1.Value - 1 {
            if (Unit_Slot_2 != 0) {
                if (Unit_Slot_2 >= 1) {
                    SendClick(x1, y1)
                    ImageFound_ability()
